...
##############################################################################################
d=read.table(file = "MarginalFoodSec.txt", header=TRUE, sep="\t")
...
p1=credplot.gg(d) +  theme_bw() + theme(axis.title.y=element_blank(),axis.title.x=element_blank(),
                                        ...
                                        axis.line = element_line(colour = "black")) +  ggtitle("Figure 1: Relative Risk Ratios (RRR) and 95% Confidence Intervals (CI) for food ")
...
# Low food security 
##############################################################################################
d=read.table(file = "LowFoodSec.txt", header=TRUE, sep="\t")
...
##############################################################################################
d=read.table(file = "VeryLowFoodSec.txt", header=TRUE, sep="\t")
